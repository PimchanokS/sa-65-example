...
func CreateRoom(c *gin.Context) {
	...
	if tx := entity.DB().Where("id = ?", room.EmployeeID).First(&employee); tx.RowsAffected == 0 { //First = เอาไอดีอันแรกที่แมทช์มาเลยไม่ต้องไปหาตัวอื่น
	...
	// 15: สร้าง Room
	WV := entity.Room{	
		Number: 	room.Number,	// ตั้งค่าฟิลด์ Number
